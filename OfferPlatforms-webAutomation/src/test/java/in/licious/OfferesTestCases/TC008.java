...
public class TC008 extends BaseTest{
	...
	// Verify the created campaign should be De-active 
		
		ReadExcelData r=new ReadExcelData();
		Map<String, String> data=r.getheaderinfo(properties.getProperty("offersCreationSheet"), "TC008");
		...
		int elementLocation=h.SelectCampaign(driver,cv.getCname(),data.get("CampaignName"));
		h.ScrollBarIntoView(driver, cv.getCampaignAction().get(elementLocation));
		h.selectDropDown(driver, cv.getCampaignAction().get(elementLocation),properties.getProperty("ActivateCampaign"));
		
		h.customWait(5);
		
		h.selectDropDown(driver, cv.getCampaignStatus(), properties.getProperty("active"));
		h.clickOnElement(driver, cv.getListCampaign());
		
		h.customWait(5);
		
		
		cname=h.GetCreatedCampiagnName(driver,cv.getCname(),data.get("CampaignName"));
		Assert.assertEquals(cname,data.get("CampaignName"));
		
		elementLocation=h.SelectCampaign(driver,cv.getCname(),data.get("CampaignName"));
		h.ScrollBarIntoView(driver, cv.getCampaignAction().get(elementLocation));
		h.selectDropDown(driver, cv.getCampaignAction().get(elementLocation),properties.getProperty("DeActivateCampaign"));
		
		
		h.customWait(5);
		
		h.selectDropDown(driver, cv.getCampaignStatus(), properties.getProperty("end"));
		h.clickOnElement(driver, cv.getListCampaign());
