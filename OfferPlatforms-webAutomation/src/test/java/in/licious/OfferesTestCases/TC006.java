...
public class TC006 extends BaseTest{
	...
	public void Tc_006() throws IOException{
		...
		// Verify the created offer should be deleted
		ReadExcelData r=new ReadExcelData();
		Map<String, String> data=r.getheaderinfo(properties.getProperty("offersCreationSheet"), "TC006");
		...
		h.selectDropDown(driver, cv.getCampaignAction().get(elementLocation),properties.getProperty("Delete"));
		 h.customWait(5);
		 cname=h.GetCreatedCampiagnName(driver,cv.getCname(),data.get("CampaignName"));
		
		 Assert.assertEquals(cname,null);			
