...
public class TC010 extends BaseTest{
	...
	public void TC_010() throws IOException{
		//	Verify created campaign  can be view and can able to edit Screen reader support enabled.

		ReadExcelData r=new ReadExcelData();
		Map<String, String> data=r.getheaderinfo(properties.getProperty("offersCreationSheet"), "TC010");
		...
		h.selectDropDown(driver, cv.getCampaignAction().get(elementLocation),properties.getProperty("View"));
		 h.customWait(5);
		 h.scrollBar(driver, cv.getEditCampaign());
		 h.enterText(driver, camp.getCampaignDesc(), "Edited Campaig");
		 h.clickOnElement(driver, cv.getUpdateCampaign());
		 h.customWait(5);
